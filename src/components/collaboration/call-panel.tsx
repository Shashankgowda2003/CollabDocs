...
export function CallPanel({
  ...
  const [fullscreenStream, setFullscreenStream] = useState<{ stream: MediaStream; name: string } | null>(null);
  ...
  const anyScreenSharing = state.screenEnabled || participants.some((p) => p.screenEnabled);
        ...
        >
          ...
          <div className="p-3 space-y-2 max-h-96 overflow-y-auto">
            <VideoTile
              ...
              isLocal
              onExpand={
                (state.screenStream || state.screenEnabled)
                  ? () => setFullscreenStream({ stream: state.screenStream ?? state.localStream!, name: localUserName })
                  : undefined
              }
            />
            ...
            {participants.map((p) => (
              <VideoTile
                ...
                isLocal={false}
                onExpand={
                  p.screenEnabled && p.stream
                    ? () => setFullscreenStream({ stream: p.stream!, name: p.name })
                    : undefined
                }
              />
            ))}
      ...
      {fullscreenStream && (
        ...
        >
          <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
            <span className="text-sm font-medium text-white">{fullscreenStream.name}&apos;s Screen</span>
            <button
              onClick={() => setFullscreenStream(null)}
              className="rounded-xl bg-white/10 hover:bg-white/20 px-3 py-2 text-xs font-semibold text-white transition-all"
            >
              Exit Fullscreen
            </button>
          </div>
    ...
    if (videoRef.current) {
      ...
      if (playPromise) {
        playPromise.catch((err) => {
          console.warn("Fullscreen video play failed:", err);
        });
      }
    }
  }, [stream]);
